...
void q1() {
    ...
    for (int i=0; i< 10; i++) {
        posterior_BadSensor = (prior_BadSensor * z_BadSensor) / (prior_GoodSensor * z_GoodSensor + prior_BadSensor * z_BadSensor);
        prior_BadSensor = posterior_BadSensor;
        prior_GoodSensor = 1.0 - prior_BadSensor;
        cout << "[INFO] The Posterior probabilics of Bad Sensor is  " << posterior_BadSensor << endl;
    }

    // 1.2: 设想住在一个白天天气为晴、多云或者雨的地方。天气转移函数是如下的转移表所示的马尔可夫链
}
